...
threshold_field = (0, 100, 1, -10, -20, 116)
...
def find_field(img):
    blobs = img.find_blobs([threshold_field], pixels_threshold=1000, area_threshold=5000, merge=True)
    if blobs:
...
def main():
    ...
    while True:
        ...
        blob_field = find_field(img)

        if blob_ball and blob_field:
            for blob in blob_ball:
                if blob_field.rect()[0] <= blob.cx() <= blob_field.rect()[0] + blob_field.rect()[2] and \
                    blob_field.rect()[1] <= blob.cy() <= blob_field.rect()[1] + blob_field.rect()[3]:
                    #Verificar si la bola esta dentro del campo 
